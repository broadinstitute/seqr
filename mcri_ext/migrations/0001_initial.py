...
class Migration(migrations.Migration):
    ...
    def update_username_match_email(apps, schema_editor):
        ...
        real_users = User.objects.using(db_alias).exclude(
            username__in=['AnonymousUser', ''])

        for user in real_users:
            logger.info('Updating user, username={}, email={}'.format(
                user.username, user.email), user=None)
    ...
    operations = [
        ...
        migrations.RunSQL(
            "ALTER TABLE oauth2_provider_accesstoken ALTER COLUMN token TYPE varchar(2048)"),
        migrations.RunSQL(
            "ALTER TABLE oauth2_provider_refreshtoken ALTER COLUMN token TYPE varchar(2048)"),

        migrations.RunPython(update_username_match_email,
