...
${INSTALL_DIR}/postgres/pgsql/bin/psql postgres < <(echo CREATE USER seqr WITH CREATEDB CREATEUSER PASSWORD \'seqr\')   # create user in case it doesn't exist yet
${INSTALL_DIR}/postgres/pgsql/bin/psql postgres < <(echo ALTER  USER seqr WITH CREATEDB CREATEUSER PASSWORD \'seqr\')   # if 'seqr' user already existed, just alter it
