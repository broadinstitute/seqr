...
# init gcloud
# Revisit: can/should we switch seqr to using workload identity instead of manually associating the pod?
...
# XXX: we want the image to be based on the version of the code rather than dynamically checking it out
if [ "$SEQR_GIT_BRANCH" ]; then
  git pull
  git checkout "$SEQR_GIT_BRANCH"
fi

# XXX: versioned docker images should preclude the need for this
...
# Revisit: see if we can either take advantage of workload identity with the cloudsql proxy, or use env vars
echo "*:*:*:*:$POSTGRES_PASSWORD" > ~/.pgpass
chmod 600 ~/.pgpass
cat ~/.pgpass

# init seqrdb unless it already exists
# Revisit: Helm/Argo give us deployment hooks that could handle migrations, but these will all
# need to be idempotent actions
...
# XXX: we should use k8s native facilities for running periodic jobs, rather than running cron in the image
...
# XXX: we should allow k8s to restart the pod if the process crashes. A health-check should help us avoid killing the 
# pod in the case of short restarts of the app
# Revisit: understand why we want to do this.
sleep 1000000000000
