...
set -x
psql -U $USERNAME postgres -c "CREATE ROLE xwiki WITH CREATEDB LOGIN PASSWORD 'xwiki'"
psql -U $USERNAME postgres -c "DROP DATABASE $DATABASE_NAME"
psql -U $USERNAME postgres -c "CREATE DATABASE $DATABASE_NAME"
psql -U $USERNAME postgres -c "GRANT ALL PRIVILEDGES ON DATABASE $DATABASE_NAME TO xwiki"
