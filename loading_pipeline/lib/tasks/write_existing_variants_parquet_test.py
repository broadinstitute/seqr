...
class WriteExistingVariantsParquetTest(MockedDatarootTestCase, ClickhouseSchemaTestCase):
    ...
    def test_snv_indel(self):
        ...
        self.assertEqual(
            ...
            [
                {'key_': 1, 'variant_id': 'a', 'geneIds': ['ENSG00000177000']},
                {'key_': 7, 'variant_id': 'c', 'geneIds': []},
                {'key_': 10, 'variant_id': 'b', 'geneIds': []},
            ],
        )

    def test_grch37_snv_indel(self):
        df = self._run_task(DatasetType.SNV_INDEL, reference_genome=ReferenceGenome.GRCh37)
        self.assertEqual(list(df.columns), ['key_', 'variant_id', 'geneIds'])
