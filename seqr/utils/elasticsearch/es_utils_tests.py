...
class EsUtilsTest(TestCase):
    ...
    def test_multi_dataset_get_es_variants(self):
        ...
        search_model.search['annotations'] = {'structural': ['DEL'], 'structural_consequence': ['MSV_EXON_OVERLAP']}
        search_model.save()
        _set_cache('search_results__{}__xpos'.format(results_model.guid), None)

        get_es_variants(results_model, num_results=5)
        self.assertExecutedSearch(
            filters=[{'bool': {'should': [{'terms': {'transcriptConsequenceTerms':
                                                         ['DEL', 'MSV_EXON_OVERLAP', 'MSV_EXON_OVR']}}, path_filter]}}],
