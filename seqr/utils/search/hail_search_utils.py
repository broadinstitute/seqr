...
def _execute_search(search_body, user, path='search', exception_map=None, user_email=None, max_retries=None):
    url = _hail_backend_url(path)

    session = requests.Session()
    if max_retries:
        session.mount(url, requests.adapters.HTTPAdapter(max_retries=max_retries))
    response = session.post(url, json=search_body, headers={'From': user_email or user.email}, timeout=300)
...
def hail_variant_multi_lookup(user_email, variant_ids, data_type, genome_version):
    ...
    response_json = _execute_search(body, user=None, user_email=user_email, path='multi_lookup', max_retries=3)
    return response_json['results']
