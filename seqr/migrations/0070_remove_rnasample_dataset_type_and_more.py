...
def _create_data_type_samples(rna_samples, rna_data, data_type, sample_ids):
    ...
    for sample in rna_samples.filter(id__in=sample_ids):
        ...
        old_id_guid_map[sample.giid] = sample.id
    ...
    id_map = {guid_old_id_map[new_model.guid]: new_model.id for old_id, new_model in new_models}

    data = rna_data.filter(sample_id__in=id_map)
    for o in data:
        o.sample_id = id_map[o.sample_id]
    rna_data.bulk_update(data, ['sample_id'], batch_size=1000)
    print(f'Updated foreign key reference for {len(data)} {data_type} data')
...
def add_data_type_rna_samples(apps, schema_editor):
    ...
    no_data_samples = rna_samples.exclude(sample_id__in=data_samples)
    if no_data_samples:
