...
from settings import SEQR_VERSION, CSRF_COOKIE_NAME, DEBUG, LOGIN_URL, GA_TOKEN_ID, ANVIL_LOADING_DELAY_EMAIL_DATE
...
def render_app_html(request, additional_json=None, include_user=True, status=200):
    ...
    should_show_loading_delay = bool(ANVIL_LOADING_DELAY_EMAIL_DATE)
    if should_show_loading_delay:
        should_show_loading_delay = datetime.strptime(ANVIL_LOADING_DELAY_EMAIL_DATE, '%Y-%m-%d') < datetime.now()
    initial_json = {'meta':  {
        ...
        'anvilLoadingDelayDate': ANVIL_LOADING_DELAY_EMAIL_DATE if should_show_loading_delay else None,
    }}
    if include_user:
