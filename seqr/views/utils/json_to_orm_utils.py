...
def update_family_from_json(family, json, user=None, session=None, allow_unknown_keys=False):
    if json.get('displayName') and json['displayName'] == family.family_id:
        json['displayName'] = ''

    update_model_from_json(
        family, json, user=user, session=session, allow_unknown_keys=allow_unknown_keys,
        immutable_keys=['pedigree_image', 'assigned_analyst'],
    )


def update_individual_from_json(individual, json, user=None, session=None, allow_unknown_keys=False):
    ...
    return update_model_from_json(
        individual, json, user=user, session=session, allow_unknown_keys=allow_unknown_keys,
        immutable_keys=[
...
def update_model_from_json(model_obj, json, user=None, session=None, allow_unknown_keys=False, immutable_keys=None):
    ...
    for json_key, value in json.items():
        ...
        if getattr(model_obj, orm_key) != value:
            if orm_key in internal_fields and not (user and is_staff(user, session)):
