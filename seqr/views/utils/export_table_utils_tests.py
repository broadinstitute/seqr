...
class ExportTableUtilsTest(TestCase):
    ...
    def test_export_table(self):
        ...
        # test tsv format
        response = export_table('test_file', header, rows, 'tsv')
        ...
        # test Excel format
        response = export_table('test_file', header, rows, 'xls')
        ...
        self.assertRaisesRegexp(ValueError, '.*format.*',
            lambda: export_table('test_file', header, rows, 'unknown_format')
        )
