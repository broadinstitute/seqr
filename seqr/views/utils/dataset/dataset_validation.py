...
from seqr.views.utils.dataset.dataset_utils import get_dataset
...
def validate_dataset(project, sample_type, analysis_type, genome_version, dataset_path, max_edit_distance=0, dataset_id=None):
    ...
    if analysis_type == Dataset.ANALYSIS_TYPE_VARIANT_CALLS:
        ...
        if len(matched_sample_id_to_sample_record) == 0:
            ...
            errors.append(("No matches found between the %(all_vcf_sample_id_count)s sample ids in the VCF and "
                         "the %(all_project_sample_id_count)s %(sample_type)s sample ids in this project") % locals())
            return errors, warnings, info
