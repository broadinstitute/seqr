...
def anvil_export(request, project_guid=None):
    if project_guid:
        projects = [Project.objects.get(project_guid=project_guid)]
    else:
        projects = Project.objects.filter(projectcategory__name__iexact='anvil')

    families = _get_over_year_loaded_project_families(projects)
    prefetch_related_objects(families, 'individual_set')

    saved_variants_by_family = _get_saved_variants_by_family(projects, request.user)

    individuals = set()
    for family in families:
        individuals.update(family.individual_set.all())
    rows = _get_json_for_individuals(individuals, project_guid=project_guid, add_family_id_field=True)
...
def _get_over_year_loaded_project_families(projects):
    ...
    return {sample.individual.family for sample in loaded_samples}
...
def _get_saved_variants_by_family(projects, user):
    ...
    for saved_variant in project_saved_variants:
        ...
        variant.update(variant_details(variant_json, saved_variant.project, user))
