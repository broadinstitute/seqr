...
@login_and_policies_required
def delete_analysis_group_handler(request, project_guid, analysis_group_guid, model_cls=AnalysisGroup, user_can_delete=True, validate_can_delete=lambda x: None):
    project = get_project_and_check_permissions(project_guid, request.user, can_edit=True)
    analysis_group = model_cls.objects.get(guid=analysis_group_guid, project=project)
    error = validate_can_delete(analysis_group)
    if error:
        raise error
    analysis_group.delete_model(request.user, user_can_delete=user_can_delete)
...
def delete_dynamic_analysis_group_handler(request, project_guid, analysis_group_guid):
    return delete_analysis_group_handler(
        request, project_guid, analysis_group_guid, model_cls=DynamicAnalysisGroup, user_can_delete=False,
