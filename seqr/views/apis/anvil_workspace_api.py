...
def anvil_workspace_page(request, namespace, name):
    ...
    """
    project_name = '{}__{}'.format(namespace, name)
    if not workspace_has_perm(request.user, CAN_EDIT, namespace, name, can_share=True):
        return {"Missing required permissions for loading data from workspace {}/{}".format(namespace, name)}
...
def create_project_from_workspace(request, namespace, name):
    ...
    try:
        ...
        info.append('Exception while sending email. {}'.format(str(ee)))
