...
class CheckNewSamplesTest(object):
    ...
    def set_up(self):
        ...
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch('seqr.utils.add_data_utils.BASE_URL', SEQR_URL)
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch('seqr.utils.add_data_utils.SEQR_SLACK_ANVIL_DATA_LOADING_CHANNEL', 'anvil-data-loading')
        patcher.start()
        self.addCleanup(patcher.stop)
        patcher = mock.patch('seqr.utils.add_data_utils.SEQR_SLACK_DATA_ALERTS_NOTIFICATION_CHANNEL', 'seqr-data-loading')
        patcher.start()
        self.addCleanup(patcher.stop)
