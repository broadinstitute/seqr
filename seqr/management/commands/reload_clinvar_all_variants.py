        ...
        with requests.get(WEEKLY_XML_RELEASE, stream=True, timeout=10) as r:
            ...
            for event, elem in ET.iterparse(gzip.GzipFile(fileobj=r.raw), events=('start', 'end',)):
                ...
                if event == 'start' and elem.tag == 'ClinVarVariationRelease':
                    ...
                    loaded_clinvar_versions = ClinvarAllVariantsSnvIndel.objects.values_list('version', flat=True).distinct()
                    ...
                    # version.
                    if existing_version_obj and existing_version_obj.version in loaded_clinvar_versions:
                        clinvar_run_sql(
