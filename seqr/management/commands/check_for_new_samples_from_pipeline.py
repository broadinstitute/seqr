...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # TODO safe get everything
        session = AirtableSession(user)
        pdos = session.get_loadable_pdos()
        for record_id, pdo in pdos.items():
            loaded_samples = set(samples_by_project[pdo['project_guid']])
            if not loaded_samples:
                continue
            skipped_samples = set(pdo['sample_ids']) - loaded_samples
            # TODO update existing PDO, create new PDO
