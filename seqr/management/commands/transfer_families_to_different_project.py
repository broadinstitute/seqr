...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        disabled_samples = backend_specific_call(lambda f: None, _disable_search)(families)
        ...
        if disabled_samples is not None:
            sample_type_dataset_type_pairs = disabled_samples.values('sample_type', 'dataset_type').distinct()
            print(sample_type_dataset_type_pairs)

            for sd in sample_type_dataset_type_pairs:
                print('in loop')
                sample_type = sd['sample_type']
                dataset_type = sd['dataset_type']
                success_message = f'Successfully deleted {len(families)} from {from_project.name} {sample_type} {dataset_type}'
                error_message = f'ERROR triggering delete families from {from_project.name} {sample_type} {dataset_type}'
