...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for gene_id, expression_array in self.get_tissue_expression_values_by_gene(
    ...
    def get_tissue_expression_values_by_gene(self, expression_file_name, samples_file_name):
        """
        ...
        tissue_type_map = self._get_tissue_type_map(samples_file_name)
        ...
        with gzip.open(expression_file_name) as expression_file:
            for i, line in tqdm(enumerate(expression_file), unit=' lines'):
                ...
                if i == 2:
                    ...
                    tissues_by_column = self._get_tissues_by_column(line, tissue_type_map)
                    continue

                fields = line.split('\t')
                gene_id = fields[0].split('.')[0]

                yield (gene_id, self._get_expressions(line, tissues_by_column))
