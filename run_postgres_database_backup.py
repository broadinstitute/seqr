...
for db_name in ['seqrdb', 'xwiki']:
   backup_filename = "seqr-v2_%(db_name)s_backup_%(timestamp)s.txt.gz" % locals()
   cmd = "pg_dump -U postgres  %(db_name)s | gzip -c - > %(backup_dir)s/%(backup_filename)s" % locals()
   print(cmd)
   os.system(cmd)

   cmd = "mv %(backup_dir)s/%(backup_filename)s /seqr-mounted-bucket/database_backups/%(backup_filename)s" % locals()
