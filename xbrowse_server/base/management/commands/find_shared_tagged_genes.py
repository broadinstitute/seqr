        ...
        for variant_tag in tqdm(VariantTag.objects.filter(), unit=' variants'):
            ...
            if variant.gene_ids is not None:
                for gene_id in variant.gene_ids:
                    ...
                    gene_to_variants[key].add("%s-%s-%s-%s" % (variant.chr, variant.pos, variant.ref, variant.alt))
            ...
            if len(gene_to_projects) % 50 == 0:
                self.print_out(gene_to_projects, gene_to_families, gene_to_variants, gene_to_variant_tags)

        self.print_out(gene_to_projects, gene_to_families, gene_to_variants, gene_to_variant_tags)

    def print_out(self, gene_to_projects, gene_to_families, gene_to_variants, gene_to_variant_tags):
        output_filename = os.path.abspath("find_shared_tagged_genes.tsv")
        print("Saving output to " + output_filename)
        with open(output_filename, 'w') as f:
        
            f.write("\t".join(['gene_id', 'gene_name', 'in_MYOSEQ_gene_list', 'gene_lists', 'tags', 'n_projects', 'projects', 'n_families', 'families', 'n_variants', 'variants']) + "\n")
