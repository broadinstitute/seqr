...
window.FamilyListTable = Backbone.View.extend({

    initialize: function(options) {
        this.families = options.families;
    },

    template: _.template($('#tpl-family-list').html()),

    // templates for the different modal views
    add_family_group_template: _.template($('#tpl-add-family-group').html()),

    events: {
        "click #select-all-families": "select_all",
        "click .family-checkbox": "select_one",
        "click #save-all-families": "save_all_families",
        "click #create-family-group": "create_family_group",
    },
    ...
    update_individual_from_html: function(m) {
        ...
        // notes
        m.set('other_notes', form_fields.filter('[data-key="other_notes"]').val());

    },

    get_selected_families: function() {
        var ret = [];
        this.$('.family-checkbox:checked').each(function(i, el) {
            ret.push($(el).data('family_id'));
        });
        return ret;
    },

    create_family_group: function() {
        var that = this;
        this.$('#base-modal-container2').html(that.add_family_group_template({
            project_id: that.options.project_id,
            family_ids: that.get_selected_families(),
        }));
        this.$('#base-modal-container2').modal();
    },

    save_all_families: function(e) {
        var that = this;
        if ($(e.target).hasClass('disabled')) return;
        $(e.target).addClass('disabled');
        that.saveCurrentForm();
        $.post(URL_PREFIX + 'project/' + that.options.project_id + '/save-all-families',
            { families_json: JSON.stringify(that.families) },
            function(data) {
                $(e.target).removeClass('disabled');
                if (data.is_error) {
                    alert('error 9923313');
    ...
    set_all_selected: function() {
        this.$('.family-checkbox').attr('checked', 'checked');
        this.$('tr.family-row').addClass('row-checked');
    },

    set_all_deselected: function() {
        this.$('.family-checkbox').removeAttr('checked');
        this.$('tr.family-row').removeClass('row-checked');
    },

    set_id_selected: function(family_id) {
        this.$('.family-checkbox[data-family_id="' + family_id + '"]').attr('checked', 'checked');
        this.$('tr.family-row[data-family_id="' + family_id + '"]').addClass('row-checked');
    },

    set_id_deselected: function(family_id) {
        this.$('.family-checkbox[data-family_id="' + family_id + '"]').removeAttr('checked');
        this.$('tr.family-row[data-family_id="' + family_id + '"]').removeClass('row-checked');
        this.$('#select-all-families').removeAttr('checked');
    },

    select_with_phenotype: function() {
        var that = this;
        this.$('#base-modal-container').html(that.select_with_phenotype_template({
            project_phenotypes: that.options.project_phenotypes,
        }));
        this.$('#base-modal-container').modal();
        this.$('#select-with-phenotype-submit').click(function() {
